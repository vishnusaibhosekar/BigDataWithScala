...
object TimeUsage {
  ...
  */
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val categories: MapView[Int, List[Column]] = columnNames
    ...
    val results = (0 to 2).map(index => categories.getOrElse(index, List.empty[Column]))
  ...
  def timeUsageSummary( primaryNeedsColumns : List[Column]
    ...
    df.select( workingStatusProjection, sexProjection, ageProjection, primaryNeedsProjection, workProjection, otherProjection).where(df("telfs") <= 4) // Discard people who are not in labor force
  ...
  /** @return SQL query equivalent to the transformation implemented in `timeUsageGrouped`
    ...
    */
  def timeUsageGroupedSqlQuery(viewName: String): String = s"SELECT working, sex, age, ROUND(AVG(primaryNeeds),1) as primaryNeeds, ROUND(AVG(work),1) as work,  ROUND(AVG(other),1) as other FROM $viewName GROUP BY working, sex, age ORDER BY working, sex, age" // { YOUR CODE HERE }
